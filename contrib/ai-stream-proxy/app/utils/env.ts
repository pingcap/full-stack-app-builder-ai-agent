...
const envSchema = z.object({
  ...
  // server
  PORT: z.coerce.number().int().max(65535).default(3000),

  REDIS_URL: z.url().regex(/^rediss?:/).default('redis://localhost:6379'),
  ...
  GC: z.enum(['0', '1']).default('1').transform(v => v !== '0'),

  // gc interval (ms)
  GC_INTERVAL: z.coerce.number().int().default(60 * 1000),
  ...
  // force gc max age (ms)
  GC_STREAM_MAX_AGE: z.coerce.number().int().default(60 * 100000),

  WEBHOOK_URLS: z.string().transform(urls => urls.split(',')).optional(),
});
